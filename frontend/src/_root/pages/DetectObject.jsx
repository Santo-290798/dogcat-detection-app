...
import { Canvas, DismissibleAlert, Popover, DownloadingAlert } from '../../components';
...
const DetectObject = () => {
    ...
    return (
        <Container style={{ marginTop: '120px' }}>
                        ...
                        <Col className='text-center'>
                            ...
                            <Row className='justify-content-center'>
                                <Col md={11} lg={9} xxl={7} className='d-flex flex-row align-items-center justify-content-center gap-3 py-2'>
                                    <Button className='upload-btn fw-semibold fs-5 my-2' onClick={focusInputFile}>
                                        <Image
                        ...
                        <Col xs={11} xxl={10} className='bg-white p-3 rounded-3 shadow-sm'>
                            ...
                            <Row className='row-gap-2'>
                                <Col className='position-relative' xs={12} lg={8} xxl={9}>
                                    ...
                                    {(!image.detectedResults && image.status !== 'failed') && <div className='loading d-flex h-100 position-absolute top-0 flex-column justify-content-center align-items-center'>
                                ...
                                <Col xs={12} lg={4} xxl={3}>
                                    <Accordion.Item eventKey={index} className='app-description fs-6'>
                                        ...
                                        {image.detectedResults ? (
                                                ...
                                                <Accordion.Body className='my-2 p-1'>
                                                    ...
                                                    {image.detectedResults.length > 0 && (
                                                        <div className='mt-2 mb-4'>
                                                            Confidence score:
                                                            <div className='conf-score overflow-y-auto'>
                                            ...
                                            <Accordion.Body className='text-danger mt-2 p-1'>Detecting objects failed!</Accordion.Body>
                                        )}
                                    </Accordion.Item>
                                </Col>
                            </Row>
            ...
            {detectedImageList.map((image, index) => (
                !image.alerted && (image.status === 'succeeded' || image.status === 'failed') && <DismissibleAlert key={index} status={image.status} imageId={image.id} />
            ))}

            {/* Show download alert */}
            {showDownloadAlert && <DownloadingAlert setShow={setShowDownloadAlert} />}
        </Container>
    );
};
